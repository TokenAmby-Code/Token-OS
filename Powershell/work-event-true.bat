@echo off
REM Stream Deck work-event button: log true_work + opportunistic ack-clear.
setlocal
...
powershell -NoProfile -Command ^
  "$evt = @{ event_type = 'work_event'; details = @{ kind = 'true_work'; source = 'stream_deck' } } | ConvertTo-Json -Compress;" ^
  "$r = Invoke-RestMethod -Uri '%API%/api/events/log' -Method POST -ContentType 'application/json' -Body $evt;" ^
  "Add-Content -Path '%LOG%' -Value (\"$(Get-Date -Format o) true_work: \" + ($r | ConvertTo-Json -Compress));" ^
  ...
  "  Add-Content -Path '%LOG%' -Value (\"$(Get-Date -Format o) true_work-ack: \" + ($a | ConvertTo-Json -Compress));" ^
  "}"
endlocal
