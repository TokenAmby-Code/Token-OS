...
```bash
curl -s -X POST "$TOKEN_API_URL/api/session-docs/<doc_id>/victory-ack" \
  -H "Content-Type: application/json" \
  -d '{"reason":"All victory criteria complete or justified skipped: <specific proof>."}'
```
