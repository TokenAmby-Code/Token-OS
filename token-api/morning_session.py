...
def gather_context() -> dict:
    ...
    ctx["yesterday_daily_note"] = _obsidian_read(f"Terra/Journal/Daily/{yesterday}.md")

    # Pax-ENV (work vault) — different path structure, no Terra/ prefix
    pax_yesterday = _obsidian_read_vault("Pax-ENV", f"Journal/Daily/{yesterday}.md")
    ...
    ctx["active_worktrees"] = (
        "\n".join(
            f"- {wt['repo']}: {wt.get('branch', 'detached')} at {wt['path']}" for wt in worktrees
        )
        if worktrees
        else "No active worktrees"
    )
    ...
    if isinstance(instances, list):
        ...
        recent = [
            i
            for i in instances
            if i.get("status") == "stopped" and i.get("last_activity", "") > yesterday
        ][:5]
        session_lines = []
        for i in active:
    ...
    if isinstance(habits, dict) and "error" not in habits:
        ...
        ctx["habits_state"] = f"Completed: {summary.get('completed', 0)}/{summary.get('total', 0)}"
    else:
