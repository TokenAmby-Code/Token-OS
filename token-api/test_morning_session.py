...
def test_run_morning_session_survives_stack_enforce_timeout(isolated_morning_dir):
    ...
    all succeeding, run_morning_session() must reach status="launched" — i.e.
    the Emperor is placed into morning-session mode despite the timeout.
    """
    ...
    with (
        ...
        # The state file is durably written as launched, so the keepalive gate
        # (morning_session_active) reports the session in-bound — i.e. the
        # Emperor is genuinely placed into morning mode, not just "launch
        # returned ok".
