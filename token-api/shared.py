...
def apply_pane_tint(
    ...
    try:
        adapter = TmuxAdapter()
        # Pane tint is decoupled from the Discord voice lock: that lock is a non-tint
        # signifier (a pane option rendered in the pane border), so persona tint
        # always paints regardless of lock state.
