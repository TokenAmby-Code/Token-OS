    ...
    try:
        conn.executescript(
            ...
            SELECT
              ...
              i.name AS tab_name,
              i.working_dir,
              CASE i.status WHEN 'working' THEN 'processing' WHEN 'implementing' THEN 'processing' ELSE i.status END AS status,
              i.engine,
              i.device_id,
              i.origin_type,
            ...
            BEGIN
              INSERT OR REPLACE INTO instances (
                ...
                stopped_at, is_questioning, questioning_since, questioning_source,
                persona_id, rank, session_doc_id, automated,
                ...
                CASE
                  ...
                  ELSE NULL
                END,
                CASE COALESCE(NEW.status, 'idle') WHEN 'processing' THEN 'implementing' ELSE COALESCE(NEW.status, 'idle') END,
                COALESCE(NEW.registered_at, NEW.created_at, CURRENT_TIMESTAMP),
                COALESCE(NEW.last_activity, NEW.created_at, CURRENT_TIMESTAMP),
                NEW.stopped_at,
                COALESCE(NEW.is_questioning, 0),
                NEW.questioning_since,
                NEW.questioning_source,
            ...
            BEGIN
              UPDATE instances SET
                ...
                status = CASE COALESCE(NEW.status, status) WHEN 'processing' THEN 'implementing' ELSE COALESCE(NEW.status, status) END,
                last_activity = COALESCE(NEW.last_activity, last_activity),
                is_questioning = COALESCE(NEW.is_questioning, is_questioning),
                questioning_since = NEW.questioning_since,
                questioning_source = NEW.questioning_source,
