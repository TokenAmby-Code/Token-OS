...
def test_session_start_uses_dispatch_pane_label_env_when_tmux_label_lookup_misses(
    app_env, monkeypatch
):
    """Dispatch-provided seat label is authoritative when @PANE_ID lookup races.

    Live failure: dispatch passed only the physical pane id. If @PANE_ID was not
    readable at SessionStart, token-api persisted pane_label=NULL and the worker
    looked unbound despite landing in the correct seat. The launch env now carries
    TOKEN_API_PANE_LABEL, and hooks.py must persist it without depending on tmux.
    """
    ...
    async def run():
        return await hooks.handle_session_start(
            {
                "session_id": "seat-bound-worker",
                "cwd": "/tmp/seat-worktree",
                "env": {
                    "TMUX_PANE": "%dispatch-target",
                    "TOKEN_API_PANE_LABEL": "palace:E",
                    "TOKEN_API_ENGINE": "claude",
                    "TOKEN_API_LAUNCHER": "dispatch",
                    "TOKEN_API_DISPATCH_TARGET": "palace:E",
                    "TOKEN_API_TARGET_WORKING_DIR": "/tmp/seat-worktree",
                },
            }
        )
    ...
    conn = _conn(app_env.db_path)
    row = conn.execute(
        """SELECT working_dir, target_working_dir, tmux_pane, pane_label, status
             FROM instances WHERE id = 'seat-bound-worker'"""
    ).fetchone()
    conn.close()

    assert row is not None
    assert row["working_dir"] == "/tmp/seat-worktree"
    assert row["target_working_dir"] == "/tmp/seat-worktree"
    assert row["tmux_pane"] == "%dispatch-target"
    assert row["pane_label"] == "palace:E"
