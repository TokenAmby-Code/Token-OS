...
STOP_EVALUATORS = {
    "action_validator": {
        "system": (
            ...
            "- The agent is telling the user to run `tx restart` (or its alias "
            "`tx -r`, or the underlying `tmuxctl restart`). That ONE command tears "
            "down the entire live tmux session — every pane and agent instance — so "
            "it is HUMAN-ONLY and the agent must NOT self-execute it; telling the "
            "user to run it is correct, not a violation. This is the ONLY workspace "
            "exception — the agent SHOULD autonomously run everything else it needs, "
            "INCLUDING `token-restart` to restart the live Token-API after code "
            "changes (do NOT flag the agent for self-restarting Token-API).\n\n"
