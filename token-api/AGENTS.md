...
**KVM Watchdog** (event-driven, Phase 1): WSL satellite manages the server side. Detection is event-driven, not polling — a `DeskflowLogFollower` tails deskflow-core's own log (`/tmp/deskflow-core.log`) and classifies connection edges (`DROP` / `UP` / `SERVER_UP`), pushing them onto a queue the watchdog consumes. The watchdog starts DeskFlow at boot, invites the Mac client (server-requests-client: "server up" ⇒ "Mac invited"), and on a `DROP` edge runs tiered recovery with a per-rung opportunistic-defer (watch the follower for a self-heal before escalating): Mac quick restart → local DeskFlow reload → full local restart → Mac client reload/restart. Failed recovery backs off exponentially and eventually enters `ceased` until an `UP` edge or manual/signal intervention. The only surviving timer is a low-frequency process-liveness check. **The Mac runs no client-side probe/supervisor** — that earlier design opened a bare TCP socket to port 24800 every 30s, which deskflow-core rejected as a failed TLS handshake (`ERROR: failed to accept secure socket` spam). The Mac is now a deterministic invite receiver: `POST /api/kvm/start` converges a known-good client (stop+start, since native auto-retry is disabled so a running-but-disconnected client won't re-dial), and Mac KVM start/reload runs `Shell/deskflow-keymap-guard.sh`, which pins the macOS input source to Australian and keeps `languageSync=false` (the authoritative fix for the recurring `'` → `:` keymap regression). Pairing is strictly 1:1 (`TokenPC` ↔ `Tokens-Mac-Mini`). Replaces the old "Deskflow" Windows scheduled task (now disabled). Vault reference: `Terra/Ultramar/Personal-Infra/Deskflow KVM.md`.
