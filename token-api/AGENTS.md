...
**KVM Watchdog** (event-driven, Phase 1): WSL satellite manages the server side. Detection is event-driven, not polling — a `DeskflowLogFollower` tails deskflow-core's own log (`/tmp/deskflow-core.log`) and classifies connection edges (`DROP` / `UP` / `SERVER_UP`), pushing them onto a queue the watchdog consumes. The watchdog starts DeskFlow at boot — spawned into its own transient systemd user unit (`deskflow-core-server` via `systemd-run --user --collect`), never as a satellite child, so satellite restarts (every deploy) cannot take the live KVM session down with the cgroup — and invites the Mac client (server-requests-client: "server up" ⇒ "Mac invited"), and on a `DROP` edge runs tiered recovery with a per-rung opportunistic-defer (watch the follower for a self-heal before escalating): Mac quick restart → local DeskFlow reload → full local restart → Mac client reload/restart. Failed recovery backs off exponentially and eventually enters `ceased` until an `UP` edge or manual/signal intervention. There is NO periodic liveness poll (removed 2026-06-10 by decree): a silent server death is caught at the next satellite boot (every deploy) or by manual `/kvm/control start` — a KVM drop is immediately user-visible, so a 5-min background check added latency-bounded noise, not safety. The only timed wake-ups are the bounded recovery retries while actively un-connected. **The Mac runs no client-side probe/supervisor** — that earlier design opened a bare TCP socket to port 24800 every 30s, which deskflow-core rejected as a failed TLS handshake (`ERROR: failed to accept secure socket` spam). The Mac is now a deterministic invite receiver: `POST /api/kvm/start` converges a known-good client (stop+start — a stale-socket client can wedge, so "already running" must not no-op). Caveat (verified 2026-06-10 from Mac tailscaled `open-conn-track` lines): despite the design intent that native auto-retry be disabled, the Mac deskflow-core client still re-dials the server every ~3s for as long as the server is down — the spec'd "try, back off after timeout, wait for the WSL invite" client behavior is NOT implemented; the watchdog tolerates the re-dial (it skips the invite when already connected) and it often reconnects faster than the ladder. Also, Mac KVM start/reload runs `Shell/deskflow-keymap-guard.sh`, which pins the macOS input source to Australian and keeps `languageSync=false` (the authoritative fix for the recurring `'` → `:` keymap regression). Pairing is strictly 1:1 (`TokenPC` ↔ `Tokens-Mac-Mini`). Replaces the old "Deskflow" Windows scheduled task (now disabled). Vault reference: `Terra/Ultramar/Personal-Infra/Deskflow KVM.md`.
