...
def request_urls(macro: dict) -> list[str]:
    return [a["requestConfig"]["urlToOpen"] for a in actions(macro, "HttpRequestAction")]


def request_bodies(macro: dict) -> list[str]:
    return [a["requestConfig"].get("contentBodyText", "") for a in actions(macro, "HttpRequestAction")]


def token_os_global(name: str) -> dict:
    matches = [v for v in load_wrapper(name).get("globalVariables", []) if v.get("m_name") == TOKEN_OS_BASE_VAR]
...
def test_controls_notification_uses_direct_button_actions_not_button_triggers() -> None:
    ...
    assert [t["m_classType"] for t in macro["m_triggerList"]] == ["HttpServerTrigger"]
    notification = actions(macro, "NotificationAction")[0]
    buttons = notification["notificationActionButtons"]
    assert [b["label"] for b in buttons] == ["Pause", "Resume", "Skip", "Faster", "Stop"]
    for button in buttons:
        ...
        assert action["requestConfig"]["urlToOpen"] == f"http://127.0.0.1:7777/tts-control?command={command}&source=notification"
...
def test_control_ingress_forwards_to_token_os_first_and_does_not_mutate_locally() -> None:
    ...
    assert token_os_global("02-control-ingress.macro")["m_stringValue"] == "http://100.95.109.23:7777"
    ...
    serialized = json.dumps(macro)
    for class_type in ["SpeakTextAction", "SetVariableAction", "CancelActiveMacroAction", "ForceMacroRunAction", "ControlMediaAction"]:
...
def test_local_control_echo_is_private_execution_authority() -> None:
    ...
    assert "macos_say" not in serialized.lower()
    assert "mac say" not in serialized.lower()


def test_chunk_player_uses_global_current_next_and_async_backfill_helper() -> None:
    macro = load("04-chunk-player.macro")
    assert token_os_global("04-chunk-player.macro")["m_stringValue"] == "http://100.95.109.23:7777"
    ...
    assert [a["m_macroName"] for a in actions(macro, "ForceMacroRunAction")] == ["01 TTS Controls Notification", "05 TTS Backfill Fetcher", "05 TTS Backfill Fetcher"]
    serialized = json.dumps(macro)
    ...
    assert request_urls(macro) == [f"{TOKEN_OS_BASE}/api/tts/chunk-event"] * 4
...
def test_error_report_goes_up_to_token_os_and_has_no_mac_fallback() -> None:
    ...
    assert token_os_global("06-error-report.macro")["m_stringValue"] == "http://100.95.109.23:7777"
    assert request_urls(macro) == [f"{TOKEN_OS_BASE}/api/tts/backend-error"]
    assert "macos" not in json.dumps(macro).lower()
    assert "mac say" not in json.dumps(macro).lower()
