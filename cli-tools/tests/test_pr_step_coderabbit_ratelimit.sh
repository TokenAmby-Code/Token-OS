...
# --- Review-return plan injection must be inert plain text -------------------
# Slash-command tokens in the injected prompt plan-cycle the receiving seat
# (derailed the Custodes seat four times on 2026-07-20). The prose survives
# ("plan ... review returned", "postplan"), but no token may carry a leading
# slash that would invoke a skill or trigger plan mode.
curl_capture="$(mktemp)"
...
grep -qF 'postplan' "$curl_capture"
if grep -qF '/plan' "$curl_capture"; then
    echo 'FAIL: review injection must not contain a /plan slash-command token'; exit 1
fi
if grep -qF '/postplan' "$curl_capture"; then
    echo 'FAIL: review injection must not contain a /postplan slash-command token'; exit 1
fi
: >"$curl_capture"
arm_pr_plan_followup merge 42 https://example.test/pr/42
grep -qF 'plan PR #42 merged' "$curl_capture"
if grep -qF '/plan' "$curl_capture"; then
    echo 'FAIL: merge injection must not contain a /plan slash-command token'; exit 1
fi

echo 'PASS: rate-limit skip merges; current-head findings still block; rate-limit-only blockage force-merges; plan-followup injections are inert plain text'
