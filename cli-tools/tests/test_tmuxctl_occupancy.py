...
def test_dispatch_target_guard_refuses_singleton_with_empty_instance_stamp():
    # row fields: pane, instance_id, pane_label, window, pane_pid
    adapter = OccupancyAdapter("%3\t\tlegion:custodes\tlegion\t999")
...
def test_dispatch_target_guard_allows_genuinely_empty_worker():
    adapter = OccupancyAdapter("%9\t\tmechanicus:1\tmechanicus\t1000")
...
def test_occupancy_for_pane_resolves_logical_target_before_display(monkeypatch):
    ...
    adapter = ResolvingOccupancyAdapter(
        {"%9": "%9\t\tmechanicus:1\tmechanicus\t1000"},
        {"mechanicus:1": "%9"},
    )
...
@pytest.mark.parametrize("row", ["", "%9\ttoo-few"])
...
def test_occupancy_for_pane_falls_back_to_original_target_when_resolution_fails(
    ...
    monkeypatch.setattr("tmuxctl.occupancy._active_agent", lambda pane_pid: False)
    adapter = ResolvingOccupancyAdapter({"boom": "%7\t\tmechanicus:7\tmechanicus\t1007"})
...
# row fields: pane, instance_id, pane_label, window, pane_pid, born


def _row(pane: str, *, born: str) -> str:
    return f"{pane}\t\tmechanicus:1\tmechanicus\t1000\t{born}"


def test_recently_born_worker_reads_occupied(monkeypatch):
    # An unbound worker (no @INSTANCE_ID, no live process) whose @PANE_BORN is
    # fresh is cold-starting — it must read occupied so the freelist/selection
...
def test_old_born_worker_is_available(monkeypatch):
    # A worker born long ago and now idle is a genuine free seat.
    monkeypatch.setattr("tmuxctl.occupancy._active_agent", lambda pane_pid: False)
...
def test_scan_excludes_recently_born_from_freelist(monkeypatch):
    ...
    adapter = ScanAdapter(
        [
            f"%1\t\tmechanicus:1\tmechanicus\t1001\t{fresh}",
            f"%2\t\tmechanicus:2\tmechanicus\t1002\t{old}",
        ]
    )
