...
def test_skills_sync_install_preserves_system_and_links_shared_skills(tmp_path):
    ...
    assert (tmp_path / "home" / ".codex" / "skills" / "postplan").resolve() == postplan.resolve()
    assert (tmp_path / "home" / ".agents" / "skills" / "preplan").resolve() == skill.resolve()
    assert (tmp_path / "home" / ".agents" / "skills" / "postplan").resolve() == postplan.resolve()
    assert (commands / "preplan.md").resolve() == (skill / "SKILL.md").resolve()
    assert (canonical.parent / "commands" / "preplan.md").resolve() == (
        skill / "SKILL.md"
    ).resolve()
    assert (commands / "postplan.md").resolve() == (postplan / "SKILL.md").resolve()
    assert (canonical.parent / "commands" / "postplan.md").resolve() == (
        postplan / "SKILL.md"
    ).resolve()
...
def test_skills_sync_install_prunes_stale_broken_codex_skill_symlinks(tmp_path):
    canonical = tmp_path / "canonical"
    canonical.mkdir()
    skill = _write_skill(canonical, "sample")
    home = tmp_path / "home"
    ...
    (codex_root / ".system" / "sentinel").write_text("keep", encoding="utf-8")
    (codex_root / "old").symlink_to(canonical / "old")
    (agents_root / "old").symlink_to(canonical / "old")

    install = _run(tmp_path, canonical, "--install", "--skip-commands", "--json")
    assert install.returncode == 0, install.stdout + install.stderr
    assert (codex_root / "sample").resolve() == skill.resolve()
    assert (agents_root / "sample").resolve() == skill.resolve()
    assert not (codex_root / "old").is_symlink()
    assert not (agents_root / "old").is_symlink()
    assert (codex_root / ".system" / "sentinel").read_text(encoding="utf-8") == "keep"
