"""Bounty: token-restart CD-sync watches the shared contracts package.

Terminus Stage 2 extracts the ops-cockpit TypeScript contracts into
``token-api/web/contracts/`` (`@token-os/contracts`, Zod schemas). Once the
cockpit imports from that package via a ``file:`` dep, a contracts-only change
must rebuild the committed ``token-api/ui/ops`` bundle on deploy — so the
...
Graduates with the contracts-extraction PR (Stage 2 PR B).
"""
...
pytestmark = [
    ...
    pytest.mark.xfail(
        strict=False,
        reason="bounty: contracts package not extracted yet (Terminus Stage 2 PR B)",
    ),
]
...
def test_ops_refresh_glob_covers_contracts_package() -> None:
    ...
    assert "token-api/web/contracts/*" in body, (
        "open bounty: ops bundle refresh must trigger on contracts changes "
