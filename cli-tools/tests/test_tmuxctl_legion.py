...
def test_selecting_custodes_preserves_last_focused_regiment_slot():
    adapter = FakeLegionAdapter()
    adapter.window_options["@LEGION_FOCUSED_PANE"] = "%2"

    result = enforce_stack_layout(adapter, "main:3", focused_pane="%C", focus=True)  # type: ignore[arg-type]

    assert result == "normalized legion layout main:3"
    assert ("resize-pane", "-t", "%1", "-y", str(LEGION_COLLAPSED_HEIGHT)) in adapter.commands
    assert any(command[:3] == ("resize-pane", "-t", "%2") for command in adapter.commands)
    assert adapter.window_options["@LEGION_FOCUSED_PANE"] == "%2"
...
def test_normalize_uses_stored_focus_without_reselecting_or_clearing_it():
    adapter = FakeLegionAdapter()
    adapter.window_options["@LEGION_FOCUSED_PANE"] = "%1"
    ...
    assert ("resize-pane", "-t", "%2", "-y", str(LEGION_COLLAPSED_HEIGHT)) in adapter.commands
...
def test_stack_dispatch_creates_managed_worker_and_launches_command():
    ...
    assert pane == "%N"
    assert ("set-option", "-p", "-t", "%N", "@PANE_ID", "legion:worker") in adapter.commands
