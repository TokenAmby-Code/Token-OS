...
def _pane(pane_id: str, role: str, *, state: GridState = GridState.SMALL, active: bool = False) -> PaneSnapshot:
    return PaneSnapshot(
        pane_id=pane_id,
        session_name="main",
        window_index=1,
...
def test_grid_focus_stashes_only_grid_siblings_and_preserves_side_panes():
    ...
    assert any(cmd[:2] == ("break-pane", "-d") and "-s" in cmd and cmd[cmd.index("-s") + 1] == "%3" for cmd in commands)
    ...
    assert not any("%1" in cmd and cmd[0] in {"break-pane", "move-pane", "kill-pane"} for cmd in commands)
    assert not any("%6" in cmd and cmd[0] in {"break-pane", "move-pane", "kill-pane"} for cmd in commands)
...
def test_palace_h_grid_focus_stashes_only_one_center_sibling():
    ...
    moving = [action.argv for action in plan.actions if action.argv[0] in {"break-pane", "move-pane"}]
...
def test_side_focus_widens_without_stashing_grid():
    ...
    assert not any(cmd[0] in {"break-pane", "move-pane", "kill-pane", "swap-pane"} for cmd in commands)
...
def test_grid_and_side_focus_state_can_coexist():
    ...
    plan = build_focus_plan(FakeAdapter(), window, "toggle")
    set_options = [cmd for cmd in [a.argv for a in plan.actions] if cmd[:3] == ("set-option", "-w", "-t")]
...
def test_grid_unfocus_plan_does_not_use_tiled_layout():
    ...
    assert not any(action.argv[:2] == ("select-layout", "-t") and action.argv[-1] == "tiled" for action in plan.actions)
...
def test_transient_window_cleanup_is_centralized_for_all_stash_families():
    ...
    assert adapter.killed == ["main:_stash_palace", "main:_fstash_somnium", "main:_focus_stash_palace"]
